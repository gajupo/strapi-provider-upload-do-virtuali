...
class FileLocationConverter {
  ...
  getKey(file) {
    ...
    if (filename.split('_').length > 2) {
      ...
      const mainFolder = file.split('_')[0];
      // get subfolder name from file name by _
      const subFolder = file.split('_')[1];
      // validates if config.folder is defined
      if (this.config.folders) {
        // if folder exists in [{folderName: 'folder', acl: 'key'}]
        const folderConfig = this.config.folders.find(f => f.folderName === mainFolder);
        if (folderConfig) {
          filename = `${folderConfig.folderName}/${subFolder}/${filename}`;
        }
      }
    }
  ...
  getACL(file) {
    ...
    const folderConfig = this.config.folders.find(f => f.folderName === folder);
    if (folderConfig) {
...
module.exports = {
  ...
  init: config => {
    ...
    return {
      upload: (file, extraParam) => new Promise((resolve, reject) => {
        console.log(`Uploading ${file.name}`);
        const key = converter.getKey(file);
        console.log(`Key: ${key}`);
